"""
Buyer CLI Client - PA3 version with frontend replica failover
"""
...
class BuyerClient:
    ...
    def __init__(self):
        ...
        # Build list of all buyer frontend replicas
        self.replicas = [
            f"http://{r['host']}:{r['port']}" for r in config.BUYER_FRONTEND_REPLICAS
        ]
    ...
    def provide_feedback(self):
        ...
        response = self._post("/buyer/provide_feedback", {
            ...
            'seller_id': seller_id, 'thumbs': thumbs
        })
        if response and response.get('status') == 'success':
            print("\n✓ Feedback recorded")
        else:
    ...
    def show_menu(self):
        print(f"\n{'='*50}")
        print(f"BUYER MENU (connected to replica {self.current_replica_idx})")
        print("="*50)
    ...
    def run(self):
        print("\n" + "="*50)
        print("ONLINE MARKETPLACE - BUYER CLIENT (PA3)")
        print("="*50)
        ...
        while True:
            ...
            while True:
                ...
                if choice == '1':    self.search_items()
                elif choice == '2':  self.get_item()
                elif choice == '3':  self.add_to_cart()
                elif choice == '4':  self.remove_from_cart()
                elif choice == '5':  self.display_cart()
                elif choice == '6':  self.save_cart()
                elif choice == '7':  self.clear_cart()
                elif choice == '8':  self.make_purchase()
                elif choice == '9':  self.provide_feedback()
                elif choice == '10': self.get_seller_rating()
                elif choice == '11': self.get_purchases()
                elif choice == '12':
                    self.logout()
                    break
                elif choice == '0':
